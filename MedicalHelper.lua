		...
		imgui.Begin(fa.ICON_HEARTBEAT .. " MedicalHelper "..scr.version, mainWin, imgui.WindowFlags.NoResize);
			...
			if select_menu[7] then
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.Spacing()
					imgui.TextWrapped(u8"\tСкрипт был разработан для проекта Samp-RP с поддержкой работы на сервере Revolution для облегчения работы сотрудникам больниц. Благодаря этому приложению Вы получите полный комплекс автоматизации многих действий и наслаждение от пользования.\nОбновления выходят по мере добавления нововведений и исправлений ошибок.")
					...
					imgui.TextColoredRGB("Благодарность blast.hk за методическую информацию и его пользователям в решении вопросов.")
						...
						imgui.SameLine()
						imgui.TextColoredRGB("Нашли баг или ошибку, или же хотите видеть что-то новое, напиши мнен"); imgui.SameLine(); imgui.Text(fa.ICON_ARROW_DOWN)
						imgui.SetCursorPosX(20)
						imgui.Text(fa.ICON_LINK)
						imgui.SameLine()
						imgui.TextColoredRGB("Для связи: VK: {74BAF4}vk.com/sumasad ")
							...
							if imgui.IsItemClicked(1) then print(shell32.ShellExecuteA(nil, 'open', 'https://vk.com/sumasad', nil, nil, 1)) end
							imgui.SameLine()
			...
			imgui.BeginChild("discord", imgui.ImVec2(0, 0), false)
				...
				if imgui.IsItemClicked(0) then
					print(shell32.ShellExecuteA(nil, 'open', 'https://discord.gg/HUfdUQgyVQ', nil, nil, 1))
				end
